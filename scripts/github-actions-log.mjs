...
/**
 * Print contributor-controlled text without allowing it to become a GitHub
 * Actions workflow command.
 *
 * The runner registers the resume token as a command while processing is
 * stopped, so the token must be unpredictable. Hex satisfies the runner's
 * token validation and 16 random bytes provide 128 bits of entropy.
 *
 * @param {unknown} value
